...
def ping():
    ...
    if request.method == 'POST':
        logging.info(f'data sent: {repr(request.form.to_dict(flat=False))}')
        logging.info(f'data sent: {repr(request.form.values()}')
        logging.info(f'data sent: {repr(request.form.listvalues()}')
